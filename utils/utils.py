...
def get_avg_fps(fps):
    new_fps = str(round(sum(fps)/len(fps),2) if len(fps) > 0 else 0.0)
    print("(FPS: " + new_fps + ")", end="")
    return new_fps
    
def get_avg_conf(conf):
    new_conf = str(round(((sum(conf)/len(conf))*100).tolist(),2) if len(conf) > 0 else 0) + "%"
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if gt or conf[j] > 0.3:  # 0.3 conf thresh
                    slabel = '%s' % cls if gt else '%s %.1f' % (cls, conf[j])
        ...
        # Image border
        cv2.rectangle(mosaic, (block_x, block_y), (block_x + w, block_y + h), (255, 255, 255), thickness=1.5)
